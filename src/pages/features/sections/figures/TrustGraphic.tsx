...
const TrustGraphic = () => (
  ...
  >
    {/* Source row */}
    <div className="border-line-soft mb-[0.85rem] flex items-center justify-between rounded-[4px] border border-dashed px-[0.85rem] py-[0.55rem]">
    ...
    <div className="border-accent ml-2 flex flex-col gap-[0.45rem] border-l-2 py-1 pl-[1.1rem]">
      {STAGES.map((label) => (
        <div
          key={label}
          className="text-ink flex justify-between rounded-[4px] bg-white/2 px-3 py-[0.45rem]"
        >
          <span>{label}</span>
          <span className="text-secondary-soft flex items-center gap-[0.4rem] text-[0.66rem] tracking-[0.16em] uppercase">
            <span className="bg-secondary inline-block size-[6px] rounded-full" />
            pass
          </span>
        </div>
      ))}
    ...
    {/* Audit */}
    <div className="border-warn mt-4 ml-2 rounded-r-[4px] border-l-2 bg-white/2 pt-[0.85rem] pr-[0.85rem] pb-[0.95rem] pl-[1.1rem]">
      ...
      <div className="grid grid-cols-2 gap-[0.4rem] sm:grid-cols-4">
        {FANS.map((fan) => (
          <div
            key={fan}
            className="border-line-soft bg-surface text-ink-dim rounded-[4px] border px-[0.4rem] py-[0.35rem] text-center text-[0.72rem]"
          >
            {fan}
          </div>
        ))}
    ...
    <div className="text-ink mt-4 ml-2 inline-flex items-center gap-[0.6rem] px-[0.85rem] py-[0.55rem] text-[0.78rem] tracking-[0.16em] uppercase">
      <span
        className="bg-accent inline-block size-[8px] rounded-full"
        style={{
          boxShadow:
